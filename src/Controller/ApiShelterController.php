...
class ApiShelterController extends AbstractController
{
    ...
    /**
     * List adopted animals in one shelter (status 0)
     * 
    ...
    /**
     * List of adopted animals in one shelter
     * 
    ...
    {
        // Récupération des animaux via ShelterRepository
 ...
 // $JSONShelter = $request->getContent();
  $shelter = new Shelter();
  ...
  if($image !== null)
  {
    ...
    $image->move(
      $this->getParameter('shelters_images_directory'),$nameImage.'.'.$image->guessExtension());
  }
...
/**
  ...
  //$shelter->setCreatedAt(new \DateTime());
  $shelter->setUpdatedAt(new \DateTime());
  //$shelter->setStatus($request->get('status'));
  $shelter->setDepartment($department);
